...
def main():
    ...
    if len(sys.argv) != 3:
        ...
        #So let's fix that
        typedDatabasePath = sys.argv[1] #this will pass on the path typed into a better named variable
        typedSequencePath = sys.argv[2] #this will pass on the path typed into a better named variable
    ...
    agaCount, tttCount, aatCount, tctCount, gatCount, tatCount, gaaCount, tctCount = 0, 0, 0, 0, 0, 0, 0, 0
    ...
    tctCount = longest_match(sequence, str4)
    ...
    #print(f"test subject: TCT: {tctCount} GAT: {gatCount} TAT: {tatCount}")
    #print(f"test subject: GAA: {gaaCount} TCT: {tctCount}")
    for i in range (lenRows):
        #print(f"Name {rows[i]['name']}")
        if int(rows[i]['AGATC']) == int(agaCount):
            if int(rows[i]['TTTTTTCT']) == int(tttCount):
                if int(rows[i]['AATG']) == int(aatCount):
                    if int(rows[i]['TCTAG']) == int(tctCount):
                        if int(rows[i]['GATA']) == int(gatCount):
                               if int(rows[i]['TATC']) == int(tatCount):
                                   if int(rows[i]['GAAA']) == int(gaaCount):
                                       if int(rows[i]['TCTG'] == int(tct):
                                            print(rows[i]['name'])
                                            return
